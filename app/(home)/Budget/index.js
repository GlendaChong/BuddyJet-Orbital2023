  ...
  return (
    <View styles={{ paddingTop: 200 }}>
      ...
      <Text style={styles.descriptionText}>
        You have not created this month's budget
      </Text>
    </View>
  );
};
  ...
  return (
    <View style={styles.financialTipContainer}>
      ...
      <Text style={styles.financialTipText}>
        Save regularly: Make saving a priority by setting aside a portion of
        your income each month. Start with a small amount and gradually increase
        it over time.
      </Text>
    </View>
  );
...
export default function Budget() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const checkBudget = await CheckMonthlyBudgetExist(
      selectedMonth,
      selectedYear
    );
        ...
        <View style={styles.budgetBoxContainer}>
          {categories.map((item, index) => (
            <View key={index} style={styles.categoryContainer}>
              ...
              <Text style={styles.categoryBoldText}>
                ${(monthlyBudgetIncome * item.spending).toFixed(0)}
              </Text>
